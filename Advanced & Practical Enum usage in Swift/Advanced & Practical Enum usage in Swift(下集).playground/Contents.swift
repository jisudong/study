...
extension Account: AccountCompatible {
    ...
    mutating func addFunds(amount: Int) throws {
        ...
        if newAmount < 0 {
            throw MyError.Overdraft(amount: -newAmount)
        } else if newAmount == 0 {
            self = .Empty
        } else {
...
// David Owens写过一篇文章，他觉得当前的关联值提取方式是很笨重的。在这里我对它的要旨进行下说明：为了从一个枚举中获取关联值，我们必须使用模式匹配。然而，关联值就是关联在特定枚举 case 的高效元组。而元组是可以使用更简单的方式来获取它内部值，即 .keyword 或者 .0。
...
//if case Token.Keyword(keyword: "import", offset: _) = importkey {
